...
import { useDeletePayment } from '@/api/hooks/usePayments'
...
export function DataTableRowActions<TData extends { id: string }>({
  ...
  const { mutateAsync: deletePaymentById, isPending: isDeleting } =
    useDeletePayment()
  ...
  const handleEditClick = () => {
    navigate({
      to: `/dashboard/payments/${row.original.id}/edit`,
    })
  }
  const handleViewClick = () => {
    navigate({
      to: `/dashboard/payments/${row.original.id}`,
    })
  }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      setIsLoading(true)
      await deletePaymentById(deleteId)
