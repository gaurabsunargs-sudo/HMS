...
export function TopSearchContent<TData>({
  ...
  useEffect(() => {
    ...
    dispatch(setTotalDoctors(0))
    dispatch(setSearchDoctors(debouncedSearchInput))
    ...
    <div className='flex w-full flex-col flex-wrap gap-3'>
      <div className='flex items-center justify-between'>
        ...
        <div className='flex gap-3'>
          ...
          <Button onClick={() => navigation({ to: '/dashboard/doctors/add' })}>
            Add Doctor <Plus size={16} className='ml-1' />
          </Button>
        </div>
      </div>
    </div>
