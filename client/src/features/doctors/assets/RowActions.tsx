...
import { useDeleteDoctor } from '@/api/hooks/useDoctors'
...
export function DataTableRowActions<TData extends { id: string }>({
  ...
  const { mutateAsync: deleteDoctorById, isPending: isDeleting } =
    useDeleteDoctor()
  ...
  const handleEditClick = () => {
    navigate({
      to: `/dashboard/doctors/${row.original.id}/edit`,
    })
  }
  const handleViewClick = () => {
    navigate({
      to: `/dashboard/doctors/${row.original.id}`,
    })
  }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      setIsLoading(true)
      await deleteDoctorById(deleteId)
