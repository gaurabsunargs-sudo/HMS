...
export function TopSearchContent<TData>({
  ...
  useEffect(() => {
    ...
    dispatch(setTotalPrescriptions(0))
    dispatch(setSearchPrescriptions(debouncedSearchInput))
    ...
    <div className='flex w-full flex-col flex-wrap gap-3'>
      <div className='flex items-center justify-between'>
        ...
        <div className='flex gap-3'>
          ...
          <Button
            onClick={() => navigation({ to: '/dashboard/prescriptions/add' })}
          >
            Add Prescription <Plus size={16} className='ml-1' />
          </Button>
        </div>
      </div>
    </div>
