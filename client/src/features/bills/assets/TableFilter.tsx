...
export function TopSearchContent<TData>({
  ...
  useEffect(() => {
    ...
    dispatch(setTotalBills(0))
    dispatch(setSearchBills(debouncedSearchInput))
    ...
    <div className='flex w-full flex-col flex-wrap gap-3'>
      <div className='flex items-center justify-between'>
        ...
        <div className='flex gap-3'>
          ...
          {isAdmin && (
            <Button onClick={() => navigation({ to: '/dashboard/bills/add' })}>
              Add Bill <Plus size={16} className='ml-1' />
            </Button>
          )}
        </div>
      </div>
