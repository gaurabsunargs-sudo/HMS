...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      variant='ghost'
      size='icon'
      className={cn('h-8 w-8 scale-90', className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
      {...props}
    >
      <PanelLeft className='h-4 w-4 scale-110' />
...
>(({ className, children, ...props }, ref) => {
  return (
    <div
      ...
      className={cn(
        'flex min-h-0 w-full flex-1 flex-col gap-2 group-data-[collapsible=icon]:overflow-hidden',
        className
      )}
      {...props}
    >
